...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($payment_method === 'cod') {
        ...
        if (!isset($_POST['cod_agreement'])) {
            die("Error: Please agree to the COD terms and conditions.");
        }
...
<body>
    ...
    input.value = number;
        ...
        function updateCheckoutButtonState() {
            ...
            if (!checkoutBtn) return; // Exit if button doesn't exist
            
            if (checkoutBtn.hasAttribute('disabled')) return; // Don't change if disabled due to stock issues
            ...
            if (paymentMethod.value === 'cod') {
                ...
                if (agreement && agreement.checked && otpVerified) {
        ...
        const codAgreement = document.getElementById('cod-agreement');
        if (codAgreement) {
            codAgreement.addEventListener('change', updateCheckoutButtonState);
        }
            ...
            checkoutForm.addEventListener('submit', function(e) {
                ...
                if (paymentMethod.value === 'cod') {
                    ...
                    if (!agreement || !agreement.checked) {
                        e.preventDefault();
                        alert("Please agree to the COD terms.");
                        return;
                    }
                    
                    if (!otpVerified) {
                        e.preventDefault();
                        alert("Please complete email verification for Cash on Delivery orders.");
                        const otpSection = document.getElementById('otp-section');
                        if (otpSection) otpSection.scrollIntoView();
                        return;
                    }
                    
                    // Check if verification token is set
                    const verificationToken = document.getElementById('phone-verification-token');
